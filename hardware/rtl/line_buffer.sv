...
module line_buffer #(parameter DATA_WIDTH = 16, parameter TILE_WIDTH = 32) 
  ...
  (* ram_style = "block" *) logic [DATA_WIDTH - 1:0] line_ram_0 [0:TILE_WIDTH - 1];
  (* ram_style = "block" *) logic [DATA_WIDTH - 1:0] line_ram_1 [0:TILE_WIDTH - 1];
  
  // Pointers and Latency Parameters
  logic [$clog2(TILE_WIDTH)-1:0] ptr;
  
  // Dedicated BRAM Read Registers (Synchronous Memory Read Output)
  logic [DATA_WIDTH - 1:0] ram0_dout;
  logic [DATA_WIDTH - 1:0] ram1_dout;
  logic [DATA_WIDTH - 1:0] pixel_in_d;

  // Pipeline delay before first valid 3x3 window
  localparam delay = (2 * TILE_WIDTH) + 3;
  ...
  // Sequential Processing Pipeline & BRAM Read / Write Logic (Infers Block RAM)
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      // Memory Read & Write Operations (Cascading through RAMs)
      line_ram_0[ptr] <= pixel_in;
      ram0_dout       <= line_ram_0[ptr];

      line_ram_1[ptr] <= ram0_dout;
      ram1_dout <= line_ram_1[ptr];
      ...
      ptr <= (ptr == $bits(ptr)'(TILE_WIDTH - 1)) ? '0 : ptr + 1'b1;
      
      // Latency Tracking
      if (pixel_count < $bits(pixel_count)'(delay)) begin
